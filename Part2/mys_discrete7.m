...
R=1-sin(2*pi*n/N*3)*0.15+sin(2*pi*n/N)*0.1;
R=1+sin(2*pi*n/N)*0.3;
t=n/N*pi*2/num_folds+sin(2*pi*n/N)*0.4;
...
if inside
  ...
  plot(real(z(length(z)/num_folds+1:2*length(z)/num_folds)),imag(z(length(z)/num_folds+1:2*length(z)/num_folds)),'ro');
%  plot(real(z(length(z)/num_folds*2+1:3*length(z)/num_folds)),imag(z(2*length(z)/num_folds+1:3*length(z)/num_folds)),'ko');
  ...
  z=ifft(Z)*num_folds;
  figure('Position',[0,0,800,800]);plot(real(z),imag(z),'ro');axis off;axis equal
  hold on;
  plot(real(z(1:length(z)/num_folds)),imag(z(1:length(z)/num_folds)),'bo');
